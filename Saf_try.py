...
capacite_calorifique = 10/4 * 360 #kWh pour chauffer de 1°c
...
for i in range(nb_intervalles - 1):
    ...
    contraintes_1 += [T_int[i + 1] - T_int[i] == - (1 - eta) * ( T_int[i]- temperatures_ext[i]) + #perte de temp sans action
                        (COP_normal(temperatures_ext[i]) * P_chauff[i] * dt / capacite_calorifique)  - #augmentation de la temp en mode normal
                        (COPT_reverse * P_refroid[i] * dt / capacite_calorifique)] #diminution de la temp en mode reverse
